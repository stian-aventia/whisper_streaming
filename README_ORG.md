...
`whisper_online_server.py` has the same model options as `whisper_online.py`, plus `--host` and `--port` of the TCP connection. (Warm-up file option removed; silent warm-up performed automatically for local backend.) See the help message (`-h` option).
